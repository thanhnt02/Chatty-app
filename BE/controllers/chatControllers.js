    ...
    Chat.find({ users: { $elemMatch: { $eq: req.user._id } } })
      ...
      .then(async (results) => {
        results = await User.populate(results, {
         ...
         // Add a new field to indicate whether the user is a friend
          let newResults = results.map(chat => {
          let newChat = {...chat._doc}
          const currentUser = chat.users.find(user => user._id.equals(req.user._id));
          newChat.isFriend = chat.users.some(user => currentUser.friends.includes(user._id));
          newChat.isBlocked = chat.users.some(user=>user.blockUsers.includes(currentUser._id));
          newChat.Blocked = chat.users.some(user=>currentUser.blockUsers.includes(user._id));
          return newChat;
        });
